...
const businessScheduleState = {
  openGroupKey: "school",
  selectedItemKey: "school-business",
};

export function renderBusinessScheduleSettings(rootElement) {
  rootElement.innerHTML = "";
  rootElement.append(createBusinessScheduleScreen(rootElement));
}

function rerender(rootElement) {
  renderBusinessScheduleSettings(rootElement);
}

function createBusinessScheduleScreen(rootElement) {
  const screen = createElement("main", {
    className: "center-settings-screen business-schedule-screen",
    dataset: { screen: "businessScheduleSettings", platform: "web" },
  });
  ...
  layout.append(createBusinessScheduleContent());
...
function createSettingsDrawerGroup(rootElement, group) {
  const isOpen = businessScheduleState.openGroupKey === group.key;
  const hasSelectedItem = group.items.some((item) => item.key === businessScheduleState.selectedItemKey);
  ...
  trigger.addEventListener("click", () => {
    businessScheduleState.openGroupKey = isOpen ? "" : group.key;
...
function createSettingsSubmenuButton(rootElement, group, item) {
  const isSelected = item.key === businessScheduleState.selectedItemKey;
  ...
  button.addEventListener("click", () => {
    ...
    businessScheduleState.openGroupKey = group.key;
    businessScheduleState.selectedItemKey = item.key;
    rerender(rootElement);
  });
...
function createBusinessScheduleContent() {
  const content = createElement("section", {
    className: "business-schedule-content",
    dataset: { area: "content" },
  });

  const header = createElement("div", { className: "business-schedule-title-bar" });
  header.append(createElement("h2", { textContent: "영업 & 휴무" }));
  content.append(header);

  content.append(createElement("div", {
    className: "business-schedule-surface",
    dataset: { state: "empty" },
  }));
