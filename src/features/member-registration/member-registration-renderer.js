...
function createPetForm(petForm, layoutMode, memberRegistrationState) {
  ...
  rightColumn.append(createPetSchoolClassField(petForm, memberRegistrationState));
...
function createPetSchoolClassField(petForm, memberRegistrationState) {
  const field = createElement("fieldset", {
    className: "registration-field registration-school-class-field",
    dataset: { field: "schoolClassIds" },
  });
  field.append(createElement("legend", { className: "registration-label", textContent: "소속 클래스" }));
  const schoolClassList = loadSchoolClassList();

  if (schoolClassList.length === 0) {
    field.append(createElement("p", {
      className: "registration-empty-message",
      textContent: "등록된 클래스가 없습니다.",
    }));
    return field;
  }

  const optionList = createElement("div", { className: "registration-class-options" });
  const selectedClassIds = new Set(petForm.schoolClassIds || []);
  schoolClassList.forEach((schoolClass) => {
    const isSelected = selectedClassIds.has(schoolClass.id);
    const option = createElement("label", {
      className: isSelected ? "registration-class-option is-selected" : "registration-class-option",
      dataset: {
        entity: "schoolClass",
    ...
    checkbox.addEventListener("change", () => {
      const currentClassIds = Array.isArray(petForm.schoolClassIds) ? petForm.schoolClassIds : [];
      petForm.schoolClassIds = checkbox.checked
...
function createGuardianEditPage(memberRegistrationState) {
  const page = createElement("main", { className: "app-page-shell has-footer guardian-edit-page", dataset: { screen: "guardianEdit" } });
...
function createMemberForRegistration(memberRegistrationState) {
  ...
  return {
    ...
    pets: memberRegistrationState.petForms.filter(isPetFormReady).map((petForm) => ({
      ...
      schoolClassIds: Array.isArray(petForm.schoolClassIds) ? [...petForm.schoolClassIds] : [],
    })),
  };
}
